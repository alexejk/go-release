...
Modified to support formatting around log levels, etc.
*/
...
const defaultLogLevelFormat = "%5s"
...
type PrefixedTextFormatter struct {
	...
	// Log level format to use for display.
	...
	// Set to true to ensure first letter of the message is capitalized.
	CapitalizeMessage bool
...
func (f *PrefixedTextFormatter) init(entry *logrus.Entry) {

	if f.CapitalizeMessage && entry.Message != "" {

		s := entry.Message
		r, n := utf8.DecodeRuneInString(s)
		entry.Message = string(unicode.ToUpper(r)) + s[n:]
	}
...
func (f *PrefixedTextFormatter) checkIfTerminal(w io.Writer) bool {
...
func (f *PrefixedTextFormatter) SetColorScheme(colorScheme *ColorScheme) {
...
func (f *PrefixedTextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	logLevelFormat := f.LogLevelFormat
	if logLevelFormat == "" {
		logLevelFormat = defaultLogLevelFormat
	}
	level := levelColor(fmt.Sprintf(logLevelFormat, levelText))
	prefix := ""
...
func (f *PrefixedTextFormatter) needsQuoting(text string) bool {
...
func (f *PrefixedTextFormatter) appendKeyValue(b *bytes.Buffer, key string, value interface{}, appendSpace bool) {
	b.WriteString(key)
	b.WriteByte('=')
...
func (f *PrefixedTextFormatter) appendValue(b *bytes.Buffer, value interface{}) {
